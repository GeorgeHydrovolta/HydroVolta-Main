...
the diluate/concentrate/electrolyte tanks (each its own chapter, since
they hold very different streams under very different conditions), and
telemetry --- each with its own specifications, operating notes,
...
(\cref{ch:tankdiluate,ch:tankconcentrate,ch:tankelectrolyte}); a communication timeout
...
\section{The CIP reservoir: water type, chemicals and dangers}
\label{sec:ciptank}
No document identifies a separate, dedicated CIP tank --- the concentrate
tank above is the one and only vessel used for CIP, per HydroVolta's own
CIP procedure (\cref{ch:dosing}). What is \emph{in} the tank changes
completely depending on which mode the unit is in, and confusing the two
is a real hazard, so treat this section as a supplement to
\cref{ch:dosing}, read from the tank's point of view.

\begin{itemize}
\item \textbf{During Production mode:} the tank holds
  \textbf{concentrate/brine} --- rejected feedwater ions, increasingly
  concentrated over multiple passes (\emph{Overview and function} above).
  This is process water, not a cleaning chemical.
\item \textbf{During the CIP acid step:} the tank holds
  \textbf{pretreated feedwater dosed with HCl to pH~2.5}
  (\cref{ch:dosing}) --- an acidic solution, corrosive at that pH.
...
\item \textbf{During the CIP hypochlorite step:} the tank holds
  \textbf{feedwater dosed with NaOCl to 2\,ppm} (sodium hypochlorite,
  effective only near pH~7--7.5) --- an oxidizing, corrosive solution.
\item \textbf{During the NaOH variant of CIP} (electrolyte-compartment
  cleaning): the tank holds \textbf{NaOH dissolved only in pure
  (permeate-quality) water} --- \textbf{never} feedwater or process
  water, per the warning in \cref{ch:dosing} (mixing NaOH with the
  minerals present in feed/process water causes scaling).
\end{itemize}
...
sampled, or drained during or immediately after a CIP cycle.

\textbf{Cleaning the tank itself:} the documented procedure already
\emph{is} the tank-cleaning procedure --- there is no additional physical
scrub-down step on file. In order: drain the acid step, flush with fresh
feed water 15--20\,minutes; drain the hypochlorite step 5\,minutes, then
flush with fresh feed water 30\,minutes (\cref{ch:dosing}). TODO: no
document describes a physical inspection (scale/residue check on the
tank walls, fittings, level sensor) as part of or after this cycle ---
until one exists, a visual check after the final flush (before returning
to Production mode) is good practice.
