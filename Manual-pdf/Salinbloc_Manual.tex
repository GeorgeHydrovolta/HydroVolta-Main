...
electrodialysis unit for water desalination and brine valorization. The
standard product is a single container fitted with the stack, dosing and
control equipment; an optional second container adds pretreatment ahead
of it for sites whose raw water needs filtering first
(\cref{ch:overview,ch:pretreatment}). Part~I of this manual
...
the diluate, concentrate, CIP and electrolyte tanks (each its own
chapter, since they hold very different streams --- and, for the CIP
tank, very different chemicals at different times --- under very
different conditions), and telemetry --- each with its own
specifications, operating notes,
...
+32~28~800~336. Web:~\href{https://hydrovolta.com}{hydrovolta.com}. See
\cref{sec:whentocallservice} for when to contact service.
...
\hvfig{fig-container-exterior.png}{Exterior view of a delivered \salinbloc
  container on site.}{fig:container-exterior}
...
The \textbf{standard \salinbloc unit is a single container} (20\,ft or
40\,ft, High-Cube or Standard) housing the \sonixed skid(s)
(\cref{ch:pumps,ch:stacks,ch:valves,ch:cabinet}) and everything downstream
of the feed connection --- it does \textbf{not} include a pretreatment
stage, so feed water must already meet the stack's feedwater envelope
(\cref{ch:stacks}) before it reaches the unit. \textbf{Pretreatment
(\cref{ch:pretreatment}) is an optional add-on}, not part of the standard
product: where a site's raw water needs filtration first, HydroVolta can
supply a \emph{second, separate pretreatment container}, connected
hydraulically ahead of the standard unit. Everything below describes the
standard single-container unit unless a step is explicitly marked as
part of the optional pretreatment container.
...
\textbf{Standard unit (no pretreatment):}
\begin{procedure}
\item Feed water enters the container directly (2" connection) --- it
  must already meet the stack's feedwater envelope (\cref{ch:stacks});
  the unit does not filter or otherwise condition it first.
\item Feed water is split and pumped through parallel \sonixed stacks
  ...
  feeding the electrode compartments, \cref{ch:tankelectrolyte}).
\item Product (permeate) water exits the container via a 2" outlet
  connection.
\item Reject/concentrate is routed to a drain tank via a 3" drainage
  connection.
\end{procedure}
\textbf{With the optional pretreatment container added} (only relevant
if that add-on was ordered, \cref{ch:pretreatment}): feed water instead
enters the separate pretreatment container first (2\textonehalf"
connection from source); disc filtration and ultrafiltration condition
it there; filtered water then leaves via a 2" connection into the main
unit's feed inlet above, and the pretreatment container's own backwash/
drainage is tee'd together with the main unit's drain line to the shared
drain tank.
...
\begin{itemize}
\item \textbf{Pretreatment and filtration skid (optional add-on, not
  part of the standard unit)} --- \cref{ch:pretreatment}
...
\item \textbf{Diluate tank} --- \cref{ch:tankdiluate}
\item \textbf{Concentrate tank} --- \cref{ch:tankconcentrate}
\item \textbf{CIP tank} --- \cref{ch:tankcip}
\item \textbf{Electrolyte tank} --- \cref{ch:tankelectrolyte}
...
The standard unit is a single ISO shipping container (\cref{ch:overview}),
20\,ft/40\,ft, High-Cube or Standard. If the optional pretreatment
add-on is included, the site needs space for a \textbf{second} container
alongside it --- in HydroVolta's documented BASF pilot, the pretreatment
unit used a 10\,ft container. The site must provide a water source
(the pilot drew from a cooling-tower basin via a submersible pump) and
space for a drain tank. TODO: ambient temperature range, ground/
...
Where the optional pretreatment container is also fitted: from the piping documented for commissioning
(\cref{sec:watercx}), its filtered-water outlet (2") must be within
pipe-run distance of the main unit's feed inlet (2"), and both
containers' 3" drain outlets should be positioned so they can be tee'd
together to a shared drain tank.

\section{Water connections}
\label{sec:watercx}
\textbf{Standard unit:}
\begin{itemize}
\item Feed inlet: 2" (M) --- feed water must already meet the stack's
  feedwater envelope (\cref{ch:stacks}) before connection.
\item Product-water outlet: 2" (M).
\item Drain outlet: 3" (M).
\item Fit O-rings and PTFE tape at every threaded joint.
\end{itemize}
\textbf{If the optional pretreatment container is also fitted}
(\cref{ch:pretreatment}) --- these connections apply only to that add-on,
not to the standard unit above:
\begin{itemize}
\item Pretreatment feed inlet 2\textonehalf" (M) from the raw water
  source (in the pilot, a submersible pump rated 4.5\,m\textsuperscript{3}/h
  at 6\,bar); filtered-water outlet 2" (M), connected to the standard
  unit's feed inlet above; drain outlet 3" (M).
\item Pump-to-pretreatment feed line: PVCu, 60\,mm, terminated 2\textonehalf" (F).
\item Pretreatment-to-unit line: PVCu, 40\,mm.
\item Both containers' 3" (M) drain outlets combine via a tee into one
  line to the shared drain tank.
\item Key in-line pretreatment components: an ultrafiltration unit
  (AKKIM APM~80) via 2" Victaulic couplings, and disc filters (AZUD
  DF100/DF50/DF20) via 2" (F) 63\,mm transition pieces.
\end{itemize}
...
The control cabinet is IP55-rated. The figures most likely to apply to
the \textbf{standard unit} come from a separate electrical schematic
series for a \textbf{treatment container} under the BASF pilot project
(folder \texttt{2\_Electrical/0\_Treatment container}, multiple dated
AutoCAD Electrical \texttt{.dsn} revisions) --- TODO: this series was
located but not opened (CAD format); a human with AutoCAD Electrical
should open the latest revision for the breaker/RCD sizing. The only
numeric figures actually extracted so far are from the \emph{pretreatment}
container's schematic instead (3$\times$400\,V\,AC, 3P+N+PE per
IEC~60445, max.\ 25\,A, 24\,V\,DC control bus for valves/I/O, sheet~E1/
document~I2) --- since pretreatment is an optional add-on
(\cref{ch:overview,ch:pretreatment}), \textbf{do not assume these values
apply to the standard single-container unit} until the treatment-container
schematic above is actually opened and checked.
...
\item Confirm all hydraulic connections listed in \cref{sec:watercx} are
  complete: feed inlet/product outlet/drain (standard unit), plus ---
  only if the optional pretreatment container was ordered --- its own
  feed/outlet/drain and the combined drain-to-tank tee.
...
\textbf{Standard unit:} connect the feed source to the 2" feed inlet and
verify the product-water outlet is connected per site requirements.
\textbf{If the optional pretreatment container is fitted:} install the
submersible feed pump in the source water body; connect its outlet to the
pretreatment 2\textonehalf" (F) inlet with 60\,mm PVCu pipe; connect the
pretreatment filtered-water outlet to the main unit's feed inlet with
40\,mm PVCu pipe; join both containers' drain outlets via a tee to the
drain tank.

\section{First power-up}
What \emph{does} exist is a real
...
\end{procedure}
See the relevant Part~II chapters for the treatment (\sonixed/stack) side,
pumps, and sensors.

\section{Handover}
Leave a copy of this manual and the unit's quality report on site with
the customer.
...
tank (\plctag{TE\_Level}) --- see
\cref{ch:tankdiluate,ch:tankconcentrate,ch:tankelectrolyte,ch:sensors}.
Additional electrical metering is available via power-meter tags
(\plctag{PM3150}).
...
in a defined sequence rather than an uncontrolled one.
...
$\rightarrow$ \cref{ch:sensors} or the relevant tank chapter
(\cref{ch:tankdiluate,ch:tankconcentrate,ch:tankcip,ch:tankelectrolyte});
...
feedwater quality, pump duty point and pipe sizing. See \cref{ch:about}
...
\chaptersubtitle{Optional add-on --- not part of the standard \salinbloc unit}
\chaptertoc

\begin{notice}
\textbf{This chapter only applies if the optional pretreatment container
was ordered alongside the unit.} The standard \salinbloc product is a
single container with no pretreatment stage --- feed water must already
meet the stack's feedwater envelope (\cref{ch:stacks}) on its own. If no
pretreatment container was supplied, skip this chapter entirely.
\end{notice}

\section{Overview and function}
Where fitted, the pretreatment container conditions raw feed water before
it reaches the \sonixed stacks: disc filtration removes coarse suspended
solids, and an ultrafiltration (UF) stage removes fine particulates and
reduces fouling potential. Filtered water passes to the main unit
...
\item \textbf{Disc filtration:} AZUD Helix Automatic self-cleaning disc
  ...
  positions DF100/DF50/DF20 by micron rating.
...
for 1\,hour, then drained until the filtrate runs chlorine-free. See
\cref{ch:maintenance} for the general log template.
...
target flow/pressure. Manual speed override, if available, is reached via
the \plctag{Manual} mode described in \cref{ch:operation}.
...
Process values (pressure, flow, conductivity per stream) are monitored via
the sensors described in \cref{ch:sensors}.
...
the most likely root causes of fouling-related performance loss.
...
component library as candidates for this duty:
\begin{itemize}
...
\section{Maintenance}
See \cref{ch:maintenance} for the general schedule and log template.
...
\item Candidate dosing pump: \textbf{Etatron GB eOne} series --- flow rate
  ...
  \textbf{PLUS} variant integrates a pH/ORP/Cl controller.
\end{itemize}
...
confirmed purchase order or as-built BOM.
\begin{itemize}
...
\item Distributed I/O: Advantech ADAM series. The \textbf{ADAM-3600-C2G}
  ...
  4--20\,mA sensors described in \cref{ch:sensors}.
...
\section{Maintenance}
See \cref{ch:maintenance} for the general schedule (dust/filter check on
the IP55 enclosure, terminal torque check) and log template.
...
\chapter{Diluate Tank}
\label{ch:tankdiluate}
\chaptersubtitle{Buffers the desalinated stream through recirculation, on its way to the customer}
\chaptertoc

\section{Overview and function}
The diluate tank (tag prefix \textbf{TD}) buffers the desalinated stream
as it recirculates through the \sonixed stack(s) (\cref{ch:stacks}) ---
diluate is not necessarily delivered in a single pass; it recirculates
through this tank and back through the stack until the target product
conductivity is reached, at which point it passes on to the product/
delivery side of the system (tag prefix \textbf{TB}, for
\enquote{breaking} out of the recirculation loop) for delivery to the
customer. This is the tank whose reading is the unit's headline water-
quality result: the HMI Overview screen's \enquote{product conductivity}
KPI (\cref{ch:operation}) is read from this stream.

\hvfig{fig-diluate-tank.png}{Suggested figure: photograph of the diluate
  tank(s), labeled with their connections (from-stack, to-stack
  recirculation, to-product-tank outlet) and instrumentation.}
  {fig:diluate-tank}
...
placed by HydroVolta give a concrete sense of the sizes used: a Rotterdam
Plastics order (Oct.\ 2025) for one 800-litre and one 450-litre vertical
polyethylene tank, and a Boralit/Galo Tanks order for two 1000-litre
rectangular polyethylene tanks (RT1000RH~ZW series). TODO: which of these
is the diluate tank was not confirmed in the order documents --- do not
assume a mapping without checking the as-built P\&ID for the specific
unit. Some documented configurations use \textbf{two} diluate buffer
tanks in series (tagged \textbf{BTD1}/\textbf{BTD2}, distinct from the
main TD tank) rather than one --- confirm which arrangement applies to
the unit at hand.
\begin{itemize}
\item Level: \plctag{TD\_Level} (main tank); \plctag{BTD1}/\plctag{BTD2}
  level alarms exist in some configurations.
\item Process instrumentation on this stream: pH (\plctag{TD\_pH}),
  pressure (\plctag{TD\_pressure}), flow (\plctag{TD\_Flow}) --- see
  \cref{ch:sensors} for sensor makes/models.
\item Downstream product tank (TB): conductivity (\plctag{TB\_Cond}) and
  level (\plctag{TB\_Level}) --- this is the KROHNE SMARTPAT COND reading
  behind the HMI's \enquote{product conductivity} KPI
  (\cref{ch:sensors,ch:operation}).
\end{itemize}

\section{Operation}
Diluate recirculates through this tank and the stack under automatic
control during \textbf{Production} mode (\cref{ch:operation}); level is
monitored continuously with high/low alarms confirmed at
\textbf{40\,cm~(low) / 90\,cm~(high)} in a documented configuration (see
the warning-code table in \cref{ch:appparams}) --- TODO confirm these
setpoints for the standard product's tank sizing. Product only passes to
the TB (delivery) tank once conductivity is within its target range.

\section{Maintenance}
TODO: no tank cleaning/inspection schedule was found. Because this tank
holds treated (low-salinity) water rather than raw feed or concentrate,
the main risk during idle periods is biological growth (algae/biofilm)
rather than scaling --- inspect at the interval in
\cref{ch:maintenance} once one is confirmed.

\section{Troubleshooting}
\plctag{HL\_Alarm\_TD}/\plctag{LL\_Alarm\_TD} cover the main tank (plus
equivalent alarms for BTD1/BTD2 where fitted). On any level alarm, check
the relevant level sensor per the bench-test procedure in
\cref{sec:sensorcal} before assuming a process fault. If product
conductivity (TB) is out of range while the tank level itself is normal,
the fault is more likely in the stack's reversal timing or feedwater
quality (\cref{ch:stacks}) than in the tank itself.


\chapter{Concentrate Tank}
\label{ch:tankconcentrate}
\chaptersubtitle{Buffers and recirculates the brine/reject stream, and doubles as the CIP reservoir}
\chaptertoc

\section{Overview and function}
The concentrate tank (tag prefix \textbf{TC}) buffers the brine/reject
stream as it recirculates through the \sonixed stack(s)
(\cref{ch:stacks}): rejected ions accumulate in this stream over multiple
passes, increasing its conductivity, until it is drained for disposal or
routed onward for brine valorization (\cref{ch:overview}). This tank also
does double duty as the \textbf{circulation reservoir during CIP
cleaning} --- HydroVolta's documented CIP procedure (\cref{ch:dosing})
explicitly circulates acid and hypochlorite cleaning solution through
this same tank, which is why the concentrate tank's minimum level is a
precondition for starting a CIP cycle.

\hvfig{fig-concentrate-tank.png}{Suggested figure: photograph of the
  concentrate tank, labeled with its connections (from-stack, to-stack
  recirculation, drain/valorization outlet, CIP circulation port).}
  {fig:concentrate-tank}

\begin{note}
This tank's role as the CIP cleaning reservoir --- what water/chemical is
in it at each stage, the hazards involved, and how it gets cleaned --- is
significant and safety-relevant enough to warrant its own chapter: see
\cref{ch:tankcip}.
\end{note}

\section{Key specifications}
As with the diluate tank, no finalized standard-product capacity/
material specification exists --- see \cref{ch:tankdiluate} for the two
real tank orders on file, which are candidates but not confirmed as to
role. Some documented configurations use \textbf{two} concentrate buffer
tanks in series (tagged \textbf{BTC1}/\textbf{BTC2}) rather than one.
\begin{itemize}
\item Level: \plctag{TC\_Level} (main tank); \plctag{BTC1}/\plctag{BTC2}
  level alarms exist in some configurations.
\item Conductivity: monitored via the \enquote{Concentrate Sensor Island}
  (candidate: KROHNE SMARTPAT COND, \cref{ch:sensors}).
\item Flow: concentrate-stream flow instrumentation confirmed in the
  register lists reviewed (\cref{ch:sensors}); a documented alarm exists
  for concentrate flow below \textbf{500\,l/hr} (\cref{ch:appparams}).
\end{itemize}

\section{Operation}
Concentrate recirculates through this tank during \textbf{Production}
mode, increasing in conductivity pass over pass; during \textbf{CIP} mode
(\cref{ch:dosing}), the same tank instead circulates cleaning chemical.
Do not start a CIP cycle without first confirming the concentrate tank's
level and that the CIP injection nozzle is submerged (\cref{ch:dosing}).

\section{Maintenance}
TODO: no tank cleaning/inspection schedule was found. Because this tank
carries the concentrated reject stream (and periodically, cleaning acid/
hypochlorite), scaling and chemical residue are more likely here than in
the diluate tank --- inspect and descale per \cref{ch:maintenance} once
an interval is confirmed, and always flush with fresh feed water after a
CIP cycle per the procedure in \cref{ch:dosing}.

\section{Troubleshooting}
\plctag{HL\_Alarm\_TC}/\plctag{LL\_Alarm\_TC} cover the main tank (plus
equivalent alarms for BTC1/BTC2 where fitted); a documented alarm fires
below 500\,l/hr concentrate flow. On any level or flow alarm, check the
relevant sensor per \cref{sec:sensorcal} before assuming a process fault.
If a CIP cycle fails to reach its target pH, verify the concentrate
tank's level and injection nozzle first, per \cref{ch:dosing}.


\chapter{CIP Tank}
\label{ch:tankcip}
\chaptersubtitle{The concentrate tank, in CIP mode: what's really inside it, and why that matters}
\chaptertoc

\section{Overview and function}
No HydroVolta document identifies a separate, dedicated CIP tank ---
\textbf{the concentrate tank (\cref{ch:tankconcentrate}) is the one and
only vessel used for CIP} cleaning, per HydroVolta's own documented CIP
procedure (\cref{ch:dosing}). It gets its own chapter here because what
is \emph{inside} that tank changes completely depending on which mode the
unit is in, and confusing the two is a real chemical-exposure hazard ---
read this chapter alongside \cref{ch:tankconcentrate,ch:dosing}, not
instead of them.

\hvfig{fig-cip-tank.png}{Suggested figure: the same concentrate tank as
  \cref{fig:concentrate-tank}, with the CIP dosing/circulation connections
  called out separately from the normal process connections.}
  {fig:cip-tank}

\section{Key specifications: what is in the tank, and when}
\begin{itemize}
\item \textbf{Production mode:} the tank holds \textbf{concentrate/
  brine} --- rejected feedwater ions, increasingly concentrated over
  multiple passes (\cref{ch:tankconcentrate}). This is process water, not
  a cleaning chemical.
\item \textbf{CIP acid step:} the tank holds \textbf{pretreated feedwater
  dosed with HCl to pH~2.5} (\cref{ch:dosing}) --- an acidic solution,
  corrosive at that pH.
\item \textbf{Between CIP steps:} the tank is drained and reflushed with
  \textbf{fresh feed water} for 15--20\,minutes specifically to clear the
  acid before the next chemical goes in.
\item \textbf{CIP hypochlorite step:} the tank holds \textbf{feedwater
  dosed with NaOCl to 2\,ppm} (sodium hypochlorite, effective only near
  pH~7--7.5) --- an oxidizing, corrosive solution.
\item \textbf{NaOH variant of CIP} (electrolyte-compartment cleaning):
  the tank holds \textbf{NaOH dissolved only in pure (permeate-quality)
  water} --- \textbf{never} feedwater or process water, per the warning
  in \cref{ch:dosing} (mixing NaOH with the minerals present in feed/
  process water causes scaling).
\end{itemize}
No capacity/material specification exists yet for a finalized standard
\salinbloc bill of materials --- see \cref{ch:tankdiluate,ch:tankconcentrate}
for the real tank orders on file and their (unconfirmed) role.

\section{Operation}
\begin{danger}
\textbf{Never let the acid (HCl) and hypochlorite (NaOCl) steps mix in
this tank.} Mixing an acid with sodium hypochlorite releases chlorine
gas, which is toxic even in small quantities. This is precisely why
HydroVolta's documented procedure requires a full drain and a 15--20
minute fresh-water flush between the acid step and the hypochlorite step
(\cref{ch:dosing}) --- never shorten or skip that flush to save time, and
never dose both chemicals into the tank at once. If chlorine odor is
detected during or after a CIP cycle, stop dosing immediately, ventilate
the area, and do not re-enter until it has cleared --- see
\cref{sec:firstaid} for first-aid measures for chemical/inhalation
exposure.
\end{danger}
Both the acid and hypochlorite solutions are corrosive at the
concentrations used (see the Safety Data Sheet summary in
\cref{sec:firstaid}): treat the tank, its vent, and any sample point as a
chemical-exposure risk during CIP, not just during dosing-pump refilling.
Wear the PPE specified in \cref{sec:ppe} whenever the tank is opened,
sampled, or drained during or immediately after a CIP cycle. Do not start
a CIP cycle without first confirming the tank's level and that the CIP
injection nozzle is submerged (\cref{ch:dosing}).

\section{Maintenance: cleaning the tank itself}
The documented CIP procedure already \emph{is} the tank-cleaning
procedure --- there is no additional physical scrub-down step on file. In
order: drain the acid step, flush with fresh feed water
15--20\,minutes; drain the hypochlorite step 5\,minutes, then flush with
fresh feed water 30\,minutes (\cref{ch:dosing}). TODO: no document
describes a physical inspection (scale/residue check on the tank walls,
fittings, level sensor) as part of or after this cycle --- until one
exists, a visual check after the final flush (before returning to
Production mode) is good practice. See \cref{ch:tankconcentrate} for the
tank's general (non-CIP) maintenance notes.

\section{Troubleshooting}
If a CIP cycle fails to reach its target pH (2.5 for the acid step),
verify the tank's level and injection nozzle before suspecting a dosing-
pump fault (\cref{ch:dosing}). If chlorine odor is ever detected, treat it
as an emergency per the warning above, not a troubleshooting item to work
through calmly. \plctag{HL\_Alarm\_TC}/\plctag{LL\_Alarm\_TC}
(\cref{ch:tankconcentrate}) apply during CIP exactly as they do during
Production --- do not disable or ignore them because the tank is in CIP
mode.


\chapter{Electrolyte Tank}
\label{ch:tankelectrolyte}
\chaptersubtitle{A separate loop feeding the stack's electrode compartments --- never part of the treated-water path}
\chaptertoc

\section{Overview and function}
The electrolyte tank (tag prefix \textbf{TE}) supplies a separate,
recirculating loop that feeds the \sonixed stack's \textbf{electrode
compartments} (\cref{ch:stacks}) --- this is what actually carries
current at the electrodes. Unlike the diluate and concentrate tanks
above, the electrolyte loop is \textbf{not part of the treated-water
path}: it never mixes with the feed, diluate or concentrate streams, and
is not delivered to the customer. A documented \enquote{Electrolyte
Preparation} monitoring point (its own conductivity transmitter,
distinct from the main process conductivity sensors) confirms the
electrolyte solution is prepared and monitored as its own loop, separate
from process water quality.

\begin{warning}
Electrode reactions can evolve gases. The system carries a general
\enquote{Ventilation~OFF} alarm (documented warning code~W002,
\cref{ch:appparams}) --- while that alarm is a general system warning
rather than confirmed as electrolyte-specific, adequate ventilation
matters most wherever electrolysis is taking place, i.e.\ around the
stack and electrolyte loop. TODO: HydroVolta engineering should confirm
the electrolyte chemistry and any specific gas-hazard/ventilation
requirement for this loop --- see also \cref{ch:safety}.
\end{warning}

\hvfig{fig-electrolyte-tank.png}{Suggested figure: photograph of the
  electrolyte tank and its connections to the stack's electrode
  compartments, clearly distinguished from the diluate/concentrate
  piping.}{fig:electrolyte-tank}

\section{Key specifications}
No finalized standard-product capacity/material specification exists ---
see \cref{ch:tankdiluate} for the two real tank orders on file (candidates,
role unconfirmed). A documented level-alarm configuration uses narrower
thresholds for this tank than for diluate/concentrate ---
\textbf{20\,cm~(low) / 80\,cm~(high)} (\cref{ch:appparams}) --- consistent
with this being a smaller support loop rather than a main process buffer,
though this has not been confirmed as a general rule.
\begin{itemize}
\item Level: \plctag{TE\_Level}.
\item Electrolyte-preparation conductivity: monitored separately from the
  main process streams (\cref{ch:sensors}).
\end{itemize}
TODO: electrolyte solution composition (which chemical, target
concentration) and replenishment/dosing method were not found in
HydroVolta's documentation reviewed.

\section{Operation}
The electrolyte loop recirculates continuously whenever the stack is
powered (\cref{ch:stacks}); level is monitored via \plctag{TE\_Level}
with the narrower alarm band noted above.

\section{Maintenance}
TODO: no maintenance schedule for the electrolyte loop was found. Because
this loop directly feeds the electrode compartments, its condition is
tied to electrode/membrane health --- see the stack maintenance notes in
\cref{ch:stacks} for the (also largely undocumented) membrane/electrode
service interval.

\section{Troubleshooting}
\plctag{HL\_Alarm\_TE}/\plctag{LL\_Alarm\_TE} cover this tank. An
unexpected electrolyte level drop should prompt a check for leaks at the
electrode compartment seals (\cref{ch:stacks}) before assuming a sensor
fault; verify the sensor itself per \cref{sec:sensorcal} either way.
...
\item \textbf{OQuila} was a third-party IoT consultancy HydroVolta held a
  ...
  HydroVolta's history, not necessarily the same one.
...
and valves stop in a defined sequence. If the unit is to be fully powered
down (as opposed to left in Controlled Shutdown/standby), isolate at the
incoming-supply breakers (main isolator, MCBs, RCDs) per the unit's own
electrical schematic (\cref{ch:cabinet}).

\section{Long-term storage}
No storage/preservation procedure specific to the whole \salinbloc unit
was found. Where the optional pretreatment container is fitted
(\cref{ch:pretreatment}), the UF membrane manufacturer's preservation
procedure is the best-documented reference available for that stage:
soak the UF module in a
...
Treat a restart after storage as a fresh commissioning
(\cref{ch:commissioning}).

\section{Disposal and recycling}
Component datasheets (e.g.\ dosing pumps, \cref{ch:dosing}) may carry
manufacturer disposal notes; dispose of dosing chemicals per their Safety
Data Sheets (\cref{sec:mainhazards}) and local regulations.
...
Use this table to record the as-commissioned values for this specific
installation.
...
(\cref{ch:tankdiluate,ch:tankconcentrate,ch:tankelectrolyte}) is finalized
--- these codes come from a specific pilot project's tank/stage
configuration and the numeric thresholds may not transfer directly. See
the alarm tag list in
\cref{ch:operation,ch:tankdiluate,ch:tankconcentrate,ch:tankelectrolyte,ch:dosing}
for additional alarms.
